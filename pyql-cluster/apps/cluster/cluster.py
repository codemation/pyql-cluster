async def run(server):
    ...
    if os.environ['PYQL_CLUSTER_ACTION'] == 'init':
        ...
        tables = [{table_name: server.get_table_func('cluster', table_name)} for table_name in table_list]
    ...
    async def cluster_ready(ready: dict = None, **kw):
    ...
    async def table_copy(cluster, table, out_of_sync_path, out_of_sync_token, out_of_sync_uuid, **kw):
        ...
        if rc == 500:
            ...
            r = table_pause(cluster, table, 'stop')
    ...
    async def table_sync_run(cluster=None, table=None, config=None, job=None, **kw):
        ...
        for endpoint in table_endpoints['out_of_sync']:
            ...
            async def load_table():
                ...
                r = table_pause(cluster, table, 'start', delay_after_pause=4.0)
            ...
            if cluster == pyql and table in pyql_sync_exclusions:
                ...
                r = table_pause(cluster, table, 'start', trace=kw['trace'], delay_after_pause=4.0)
